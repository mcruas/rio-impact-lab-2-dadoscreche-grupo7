...
- **`04_UnidadesEscolaresComEndereco.csv` (Query D) carries each school's own CEP** in its last column (1,930 rows with a real CEP, 1,533 distinct). Joined against the school lat/long in `Unidades_Unificadas_com_Localizacao.xlsx`, this yields ~1,900 "real CEP → real coordinate" anchors — the only geocoding source in the repo, and enough to locate ~98% of family enrollment demand by CEP prefix **without any external API**. See `modulos/recomendacao-escolas/app/ceps.py` for the worked cascade. Don't reach for a geocoding API before this: BrasilAPI was measured on 120 CEPs and returned the sentinel `-22.90642,-43.18223` (Rio city centre) for 77% of them instead of the real coordinate.
- **One school has zeroed coordinates** in `Unidades_Unificadas_com_Localizacao.xlsx`: Escola Municipal Pedro Bruno (`DESIGNACAO` 121002, Paquetá) has `LATITUDE`/`LONGITUDE` = 0. Since `0` is not null, a plain `IS NOT NULL` filter lets it through and it drags any average it participates in — the Paquetá centroid landed at (-11.38, -21.55), halfway to "null island". Treat `(0, 0)` as missing, not as a point (`modulos/recomendacao-escolas/app/data.py::_coordenada`).
...
**Derived data goes in the owning module's `dados/`, generated by a script in its `scripts/`, and is committed to git** — because `desafio/` is gitignored, a deploy (Vercel) has neither the raw data nor the DuckDB `excel`/`spatial` extensions. `modulos/recomendacao-escolas/` is the reference for the pattern: `app/data.py` and `app/ceps.py` each expose a `carregar_*()` that reads the committed artifact in production and a `recalcular_de_desafio()` that rebuilds it from `desafio/`, called only by `scripts/gerar_dataset.py` / `scripts/gerar_ceps.py`. Current artifacts: `dados/escolas.duckdb` (1,941 schools), `dados/ceps.csv` (~20.8k CEPs with coordinates and micro-area), `dados/cep_escolas.csv` (10 nearest schools per CEP). Regenerate all of them whenever `desafio/` changes. Never put data files in `contracts/schemas/` — the validator treats every `*.schema.json` there as a schema.
